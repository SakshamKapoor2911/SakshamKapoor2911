...
|---|---|---|
| **Trufflow: Graph-Aware Anomaly Engine** | Production-Grade Observability. Architected a context-aware detection engine for multi-tenant SaaS. • The Engineering: Fused time-series metrics with transaction graph topology ("Blast Radius") to solve non-stationarity.
• The Win: Achieved 73x lift in PR-AUC (0.438 vs 0.006) over baseline.
• The Scale: Optimized inference to 0.80 μs/sample, enabling >1.2M predictions/sec on commodity CPU. | `Python`, `Polars`, `XGBoost`, `Graph Theory`, `Scikit-Learn` |
